...
import WorkoutAIReport from "./WorkoutAIReport";
const DEFAULT_STATS = [
...
})
{
  ...
  return (
    <div
      dir="rtl"
      className="bg-linear-to-b from-blue-500 to-blue-600 rounded-3xl p-8 flex flex-col gap-6 w-full shadow-lg text-right"
    >
      <h3 className="text-white text-3xl">{title}</h3>

      <p className="text-blue-50 text-base leading-8 Modam-Medium">
        {description}
      </p>
      ...
      <div className="flex flex-col gap-4">
        {stats.map((item, idx) => (
          ...
          >
            ...
            <span className="text-white text-lg Modam-Medium">
              {item.value}
            </span>
          </div>
        ))}
      ...
      {isReportVisible && (
        <WorkoutAIReport onClose={() => setIsReportVisible(false)} />
      )}
    </div>
  );
}
